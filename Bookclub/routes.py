...
def search():
    ...
    if form.validate_on_submit():
        ...
        posts = posts.join(User).outerjoin(Comment).filter(
            (Post.title.ilike(f'%{search_term}%')) | #search by post title
            (Post.content.ilike(f'%{search_term}%')) | #search by post content
            (User.username.ilike(f'%{search_term}%')) | #search by username
            (Comment.content.ilike(f'%{search_term}%')) #search by comment content
